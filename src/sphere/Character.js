...
export function buildKid(loadout = DEFAULT_LOADOUT) {
  ...
  // ★ 꼬리 마디 셋을 **각자 제자리에 놓고 각자 돌리고** 있었다. 각도가
  //   −0.35 → −1.05로 벌어지는 데다 달릴 때 더 밀리니 마디 사이가 갈라져
  //   **몸 옆에 뜬 주황 파편**으로 보였다(실사용 스크린샷에서 지적받았다).
  //   사슬로 잇는다 — 마디를 윗마디의 **끝**에 매달고 경첩을 마디의 윗변에 둔다.
  //   그러면 아무리 흔들어도 절대 안 끊어진다. 천은 원래 그렇게 생겼다.
  const tail = [];
  // 폭은 확실히 좁혀 간다. 0.17→0.15→0.12는 거의 안 좁아져서 **주황 널빤지**로
  //   보였다(등판 폭 0.38의 절반). 천은 끝으로 갈수록 가벼워 보여야 한다.
  const SEG = [{ w: 0.16, h: 0.22, d: 0.045 }, { w: 0.125, h: 0.20, d: 0.04 },
               { w: 0.09, h: 0.17, d: 0.035 }];
  const REST = [0.46, -0.14, 0.04];     // animateLimbs가 안 도는 화면에서도 뒤로 늘어져 있어야
  // ★ 경첩을 z −0.08에 두니 첫 마디가 **웃옷 안에 파묻혀** 있었다(재 보니
  //   첫 마디 전체가 몸통 표면 −0.176 안쪽). 목고리 → 빈칸 → 떠 있는 조각 둘.
  //   고치기 전과 똑같이 보인다. 경첩을 등판 바깥으로 내보낸다.
  let hook = k, hx = 0, hy = 1.27, hz = -0.21, si = 0;
  for (const t of SEG) {
    const seg = new THREE.Group();
    seg.position.set(hx, hy, hz);
    seg.rotation.x = REST[si++];
    const m = box(t.w, t.h, t.d, scarf);
    m.position.set(0, -t.h / 2, 0);          // 경첩은 마디의 윗변
    seg.add(m);
    hook.add(seg);
    tail.push(seg);
    hook = seg; hx = 0; hy = -t.h; hz = 0;   // 다음 마디는 이 마디 끝에
  }
...
export function animateLimbs(k, dt, moving, running) {
  ...
  // 스카프 — 달릴수록 크게 날린다. 사슬이라 각도는 **윗마디 기준 상대값**이다.
  // 절대값으로 두면 마디가 갈라진다 — 그게 파편처럼 보이던 원인이었다.
  //
  // ★ 그리고 **부호가 반대였다.** rotation.x가 음수면 마디 끝이 +z,
  //   즉 **앞쪽**으로 간다(버클이 z +0.185에 있으니 +z가 앞이다).
  //   재기 전까지 꼬리 끝이 z +0.157에 있었다 — 앞자락(+0.10)을 뚫고
  //   **가슴으로 빠져나와 있었다.** 뒤에서 보면 안 보이니 음수로 두고도
  ...
  if (u.scarfTail) {
    ...
    for (let i = 0; i < u.scarfTail.length; i++) {
      u.scarfTail[i].rotation.x =
        base[i] + f + Math.sin(u.walkPhase * 0.8 + i * 0.9) * 0.11 * (0.3 + A);
    }
  }
}
