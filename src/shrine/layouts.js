...
export const SHRINES = [
  ...
  // 나머지 다섯이 얼마나 시끄러운지를 이 사당이 기준으로 잡아 준다.
  {
    ...
    rooms: spine({
      entryTo: 6.0,
      rooms: [
        // ★ 첫 두 방이 빛 타일·레이저였다 — 반사신경 방이고 무게와 무관했다.
        //   아이가 처음 만나는 사당이 "이 게임은 피하기 게임"이라고 가르쳤다(비평 ②).
        //   지킴이의 첫마디 "눈은 자주 틀린다. 저울은 틀린 적이 없다"가 곧 첫 방이다.
        { id: 'r1', name: '무게 순서', w: 12, len: 14, h: 5.5, gate: 'weigh', act: 'carry',
            goal: '상자 다섯을 가벼운 것부터 받침에 세워라. 크기는 믿지 마라.',
            hints: ['저울에 둘씩 올려 봐라. 어느 쪽이 무거운지만 말해 준다.',
              '큰 상자가 무거운 게 아니다. 눈이 아니라 저울이 안다.',
              '가장 가벼운 것부터 찾아라 — 모두와 비교해서 한 번도 안 무거운 것.'] },
        { id: 'r2', name: '무게 압력판', w: 12, len: 10, h: 5.5, gate: 'plate', act: 'carry',
            goal: '두 판이 주문한 무게를 만들어라. 주문은 세 번 바뀐다.',
            hints: ['판 테두리가 붉으면 너무 무거운 것이다.', '큰 상자를 먼저 놓고 남는 수를 작은 상자로 채워라.'] },
        { id: 'shrine', name: '신전', w: 18, len: 16, h: 9, act: 'carry',
            goal: '왼쪽 3kg이 2칸에 있다. 오른쪽으로 수평을 맞춰라.',
            hints: ['무게 × 칸수가 같아야 수평이 된다.', '왼쪽은 3kg이 2칸이다. 오른쪽도 그만큼.'] },
      ],
    }),
    final: 'scale',
  },
      ...
      rooms: [
        ...
        { id: 'shrine', name: '신전', w: SHADOW_SANCTUM.width, len: SHADOW_SANCTUM.length, h: 8, architecture: 'shadowSanctum', act: 'turn',
            ...
            hints: ['손잡이 하나는 방향, 하나는 길이를 바꾼다.',
              ...
              '방향을 다 맞춘 뒤에 길이를 손대라 — 과학자도 그렇게 한다.'] },
      ],
    }),
    final: 'mirrorGod',
  },
  ...
  // 높다. 수위가 오르고 발밑이 미끄럽다. 방의 모양이 시간에 따라 바뀌는 유일한 사당.
  {
    ...
    rooms: spine({
      entryTo: 6.0,
      rooms: [
        { id: 'r1', name: '얼려서 건너기', w: 10, len: 16, h: 7.0, gate: 'freeze', act: 'turn',
            goal: '물 세 줄. 줄마다 얼려서 건너라 — 갈수록 빨리 녹는다.',
            hints: ['손잡이는 줄마다 따로 있다.', '얼음이 깜빡이면 곧 녹는다. 미리 다음 손잡이로.'] },
  ...
  // 방 폭이 들쭉날쭉하다. 반듯한 방이 하나도 없어야 "무너지는 중"으로 읽힌다.
  {
    ...
    rooms: spine({
      ...
      rooms: [
        ...
        { id: 'r3', name: '간헐천', w: 13, len: 14, h: 6.5, gate: 'geyser', act: 'time',
            goal: '고리가 차오르면 뿜는다. 주기가 셋 다 다르다.',
  ...
  // 문을 지나는 순간 천장이 안 보이는 것이 이 방의 첫인상이어야 한다.
  {
    ...
    rooms: spine({
      ...
      rooms: [
        { id: 'r1', name: '층의 순서', w: 10, len: 16, h: 6.0, gate: 'strataOrder', act: 'carry',
            goal: '눕혀 둔 시추 코어를 읽고, 같은 순서로 벽에 꽂아라. 어느 끝이 위인가?',
            hints: ['시추는 위에서 아래로 뚫는다. 드릴 날이 박힌 쪽이 땅 위다.',
              '날의 반대편 끝이 가장 깊은 곳 — 그게 벽의 1층이다.'] },
        { id: 'r2', name: '내려가는 갱도', w: 8, len: 18, h: 9.0, gate: 'shaft', act: 'avoid',
            goal: '바닥에 그림자가 뜨면 그 자리로 바위가 떨어진다.',
            hints: ['바닥 그림자가 커지면 그 자리로 떨어진다.', '그림자를 보고 옆으로 한 걸음이면 된다.'] },
        { id: 'r3', name: '다섯 색의 문', w: 12, len: 14, h: 6.0, gate: 'fiveDoors', act: 'turn',
            // ★ 기억력 시험이었다. 단서는 수첩에 이미 있는데(사당마다 표지·색) 아무도
            //   쓰라고 안 했다. 이 방은 "수첩을 도구로 쓰는 방"이다.
            goal: '표지가 가리키는 사당의 색으로 손잡이를 맞춰라. 수첩에 적혀 있다.',
